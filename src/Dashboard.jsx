...
export default function Dashboard() {
  ...
  const [missingTripModal, setMissingTripModal] = useState(false)
  ...
  async function loadData(pg) {
    try {
      const res = await axios.post(`${API}/api/dashboard`, { email, page: pg })
      if (!res.data.success) { setError(res.data.message); setLoading(false); return }
      setAllRows(prev => pg === 0 ? res.data.rows : [...prev, ...res.data.rows])
      setStatsBar(res.data.statsBar || [])
      setTripActions(prev => ({ ...prev, ...res.data.actions }))
      setResolutions(prev => ({ ...prev, ...res.data.resolutions }))
      setTransporterIds(res.data.transporterIds || [])
      setVendorName(res.data.vendorName || email)
  ...
  try {
    const res = await axios.post(`${API}/api/ageing`, { email })
  ...
  async function confirmAccept() {
    ...
    try {
      const res = await axios.post(`${API}/api/accept-trip`, {
        tripId: pendingAccept.tripId,
        transporterId: pendingAccept.transporterId,
        vendorEmail: email,
        vendorName: pendingAccept.vendorName,
        amount: pendingAccept.amount
      })
      if (res.data.success) {
        setTripActions(prev => ({ ...prev, [pendingAccept.tripId]: 'accepted' }))
      }
    } catch {}
...
async function submitDispute() {
  if (!disputeCategory) { 
    alert('Please select a dispute category.'); 
    return 
  }
  if (disputeModal.trip_id === 'MISSING' && !missingTripId.trim()) { 
    alert('Please enter the missing trip ID.'); 
    return 
  }
  if (disputeCategory === 'Trip Missing') {
  if (missingTripIds.length === 0) {
    alert('Please add at least one missing trip ID.')
    return
  }
} else if (!disputeSecondary.trim()) {
  alert('Please answer the follow-up question before submitting.')
  return
}

if (disputeCategory !== 'Trip Missing' && !disputeSummary.trim()) {
  alert('Please provide an additional summary before submitting.')
  return
}
  ...
  try {
    ...
    formData.append('tripId', disputeModal.trip_id === 'MISSING' ? missingTripId.trim() : disputeModal.trip_id)
    formData.append('transporterId', disputeModal.transporter_id || transporterIds[0])
    formData.append('vendorEmail', email)
    formData.append('vendorName', disputeModal.vendor_name || vendorName)
    formData.append('amount', disputeModal.amount || 0)
    formData.append('disputeReason', fullReason)
    formData.append('disputeCategory', disputeCategory)
    formData.append('disputeSecondary', disputeSecondary)
    formData.append('disputeSummary', disputeSummary)
    formData.append('proof', proofFile)

    const res = await axios.post(`${API}/api/raise-dispute`, formData, {
...
function addMissingTripId() {
  ...
  if (!pattern.test(val)) {
    setMissingTripError('Invalid format. Use TR-XXXXXXXX-XXXX (e.g. TR-12345678-1234)')
    return
  }
  if (missingTripIds.includes(val)) {
    setMissingTripError('This trip ID has already been added.')
    return
  }
...
async function submitBulkDispute() {
  ...
  try {
    const formData = new FormData()
    formData.append('excel', bulkExcel)
    bulkProofs.forEach(f => formData.append('proofs', f))

    const res = await axios.post(`${API}/api/bulk-dispute`, formData, {
      headers: { 'Content-Type': 'multipart/form-data' }
    })

    if (res.data.success) {
      setBulkSuccess(`Successfully imported ${res.data.imported} disputes!`)
      setBulkExcel(null)
      setBulkProofs([])
    } else if (res.data.errors) {
      setBulkErrors(res.data.errors)
    } else {
      setBulkErrors([{ row: '—', trip_id: '—', error: res.data.message || 'Upload failed. Please try again.' }])
    }
  } catch (err) {
    setBulkErrors([{ row: '—', trip_id: '—', error: 'Server error: ' + (err.response?.data?.message || err.message || 'Unknown error. Please try again.') }])
  }
  setBulkLoading(false)
}

function downloadTemplate() {
  window.open(`${API}/api/bulk-dispute-template`, '_blank')
}
    ...
    <div style={{...styles.modal, maxWidth:'480px'}}>
      ...
      {disputeModal.trip_id === 'MISSING' && (
        <div style={{marginBottom:'1rem'}}>
          ...
          <input
            ...
            placeholder="Enter trip ID e.g. TR-12345678-1234"
            value={missingTripId}
            onChange={e => setMissingTripId(e.target.value)}
          />
        </div>
      )}
      ...
      <div style={{display:'flex',flexDirection:'column',gap:'0.4rem',marginBottom:'1rem'}}>
        {[
          'Wrong RFQ applied',
          'Wrong Rate',
          'Adhoc/Regular Mismatch',
          'Vehicle size mismatch',
          'Route Changed / Addition stops added',
          ...
          <label key={cat} style={{display:'flex',alignItems:'center',gap:'0.5rem',padding:'0.5rem 0.75rem',border:`1px solid ${disputeCategory===cat?'#4f46e5':'#e2e4ed'}`,borderRadius:'8px',cursor:'pointer',background:disputeCategory===cat?'rgba(79,70,229,0.05)':'#fff',fontSize:'0.85rem',fontWeight:disputeCategory===cat?600:400}}>
            <input type="radio" name="disputeCat" value={cat} checked={disputeCategory===cat} onChange={() => { setDisputeCategory(cat); setDisputeSecondary('') }} style={{accentColor:'#4f46e5'}}/>
            {cat}
          </label>
        ))}
      </div>
      ...
      {/* SECONDARY QUESTION */}
      {disputeCategory === 'Wrong RFQ applied' && (
  <div style={styles.secondaryBox}>
    <label style={styles.secondaryLabel}>⤷ Share Correct RFQ <span style={{color:'#dc2626'}}>*</span></label>
    <input style={styles.disputeInput} placeholder="Enter correct RFQ…" value={disputeSecondary} onChange={e => setDisputeSecondary(e.target.value)}/>
  </div>
)}

{disputeCategory === 'Wrong Rate' && (
  <div style={styles.secondaryBox}>
    <label style={styles.secondaryLabel}>⤷ Enter Correct Rate <span style={{color:'#dc2626'}}>*</span></label>
    <input style={styles.disputeInput} placeholder="Enter correct rate as per you…" value={disputeSecondary} onChange={e => setDisputeSecondary(e.target.value)}/>
  </div>
)}

{disputeCategory === 'Adhoc/Regular Mismatch' && (
  <div style={styles.secondaryBox}>
    <label style={styles.secondaryLabel}>⤷ Confirm Correct Trip Type <span style={{color:'#dc2626'}}>*</span></label>
    <div style={{display:'flex',gap:'0.5rem'}}>
      {['Adhoc','Regular'].map(t => (
        <label key={t} style={{display:'flex',alignItems:'center',gap:'0.4rem',padding:'0.4rem 0.75rem',border:`1px solid ${disputeSecondary===t?'#4f46e5':'#e2e4ed'}`,borderRadius:'8px',cursor:'pointer',background:disputeSecondary===t?'rgba(79,70,229,0.05)':'#fff',fontSize:'0.85rem'}}>
          <input type="radio" name="tripType" value={t} checked={disputeSecondary===t} onChange={() => setDisputeSecondary(t)} style={{accentColor:'#4f46e5'}}/>{t}
        </label>
      ))}
    </div>
  </div>
...
{disputeCategory === 'Vehicle size mismatch' && (
  <div style={styles.secondaryBox}>
    <label style={styles.secondaryLabel}>⤷ Confirm Correct Vehicle Size <span style={{color:'#dc2626'}}>*</span></label>
    <input style={styles.disputeInput} placeholder="e.g. 8, 10, 12, 14, 20…" value={disputeSecondary} onChange={e => setDisputeSecondary(e.target.value)}/>
  </div>
)}

{disputeCategory === 'Route Changed / Addition stops added' && (
  <div style={styles.secondaryBox}>
    <label style={styles.secondaryLabel}>⤷ Confirm Correct Route <span style={{color:'#dc2626'}}>*</span></label>
    <input style={styles.disputeInput} placeholder="e.g. SLS->MLB->FPJ->FV4->CA8" value={disputeSecondary} onChange={e => setDisputeSecondary(e.target.value)}/>
  </div>
)}

{disputeCategory === 'Vehicle number Mismatch' && (
  <div style={styles.secondaryBox}>
    <label style={styles.secondaryLabel}>⤷ Confirm Correct Vehicle Number <span style={{color:'#dc2626'}}>*</span></label>
    <input style={styles.disputeInput} placeholder="e.g. MH12AB1234" value={disputeSecondary} onChange={e => setDisputeSecondary(e.target.value)}/>
  </div>
)}
...
{disputeCategory === 'Trip Missing' && (
  <div style={styles.secondaryBox}>
    ...
    <div style={{fontSize:'0.72rem',color:'#d97706',marginBottom:'0.5rem'}}>Format: TR-XXXXXXXX-XXXX (e.g. TR-12345678-1234)</div>
    <div style={{display:'flex',gap:'0.5rem',marginBottom:'0.5rem'}}>
      <input
        style={{...styles.disputeInput, flex:1}}
        placeholder="TR-12345678-1234"
        value={missingTripInput}
        onChange={e => setMissingTripInput(e.target.value)}
        onKeyDown={e => e.key === 'Enter' && addMissingTripId()}
      />
      <button
        style={{padding:'0.6rem 1rem',background:'#4f46e5',border:'none',borderRadius:'8px',color:'#fff',fontFamily:'Inter,sans-serif',fontSize:'0.82rem',fontWeight:600,cursor:'pointer',whiteSpace:'nowrap'}}
        onClick={addMissingTripId}>
        + Add
      </button>
    </div>
    ...
    {missingTripIds.length > 0 && (
      <div style={{display:'flex',flexWrap:'wrap',gap:'0.4rem',marginTop:'0.25rem'}}>
        {missingTripIds.map((id, i) => (
          <div key={i} style={{display:'flex',alignItems:'center',gap:'0.3rem',background:'rgba(5,150,105,0.08)',border:'1px solid rgba(5,150,105,0.3)',borderRadius:'6px',padding:'0.2rem 0.5rem',fontSize:'0.78rem',color:'#059669',fontWeight:600}}>
            {id}
            <span style={{cursor:'pointer',color:'#dc2626',fontWeight:700,marginLeft:'2px'}} onClick={() => setMissingTripIds(prev => prev.filter((_,j) => j !== i))}>×</span>
          </div>
        ))}
      </div>
    )}
    {missingTripIds.length > 0 && (
      <div style={{fontSize:'0.72rem',color:'#6b7280',marginTop:'0.4rem'}}>{missingTripIds.length} trip ID{missingTripIds.length > 1 ? 's' : ''} added</div>
    )}
  </div>
)}
{/* ADDITIONAL SUMMARY */}
{disputeCategory && disputeCategory !== 'Trip Missing' && (
  <div style={{marginTop:'0.75rem'}}>
    <label style={styles.disputeLabel}>
      Additional Summary <span style={{color:'#dc2626'}}>*</span>
    </label>
    <textarea
      style={{width:'100%',padding:'0.6rem 0.75rem',border:'1px solid #e2e4ed',borderRadius:'8px',fontFamily:'Inter,sans-serif',fontSize:'0.85rem',color:'#1a1d2e',outline:'none',minHeight:'80px',resize:'vertical',boxSizing:'border-box'}}
      placeholder="Provide a detailed summary of the issue to help the agent resolve it faster…"
...
<div style={{marginTop:'0.75rem',padding:'0.75rem 1rem',background:'#f8fafc',border:'1px solid #e2e4ed',borderRadius:'10px'}}>
  <label style={{display:'block',fontSize:'0.75rem',fontWeight:700,color:'#1a1d2e',marginBottom:'0.4rem',textTransform:'uppercase',letterSpacing:'0.05em'}}>
    Upload Proof <span style={{color:'#dc2626'}}>*</span>
  </label>
  <div style={{fontSize:'0.72rem',color:'#6b7280',marginBottom:'0.5rem'}}>JPG, PNG or PDF — max 10MB — 1 file per dispute</div>
  <input
    type="file"
  ...
  />
  <label htmlFor="proofUpload" style={{display:'inline-flex',alignItems:'center',gap:'0.4rem',padding:'0.5rem 1rem',background:'#fff',border:'1px solid #e2e4ed',borderRadius:'8px',cursor:'pointer',fontSize:'0.82rem',color:'#4f46e5',fontWeight:600}}>
  ...
  {proofFile && (
    <div style={{display:'flex',alignItems:'center',gap:'0.5rem',marginTop:'0.5rem',padding:'0.4rem 0.75rem',background:'rgba(5,150,105,0.08)',border:'1px solid rgba(5,150,105,0.3)',borderRadius:'6px'}}>
      <span style={{fontSize:'0.78rem',color:'#059669',fontWeight:600}}>✓ {proofFile.name}</span>
      <span style={{fontSize:'0.75rem',color:'#6b7280'}}>({(proofFile.size/1024/1024).toFixed(2)}MB)</span>
      <span style={{cursor:'pointer',color:'#dc2626',marginLeft:'auto',fontWeight:700}} onClick={() => { setProofFile(null); document.getElementById('proofUpload').value=''; }}>×</span>
    </div>
  )}
  {proofError && <div style={{fontSize:'0.75rem',color:'#dc2626',marginTop:'0.4rem'}}>{proofError}</div>}
</div>
      <div style={{display:'flex',gap:'0.75rem',marginTop:'0.5rem'}}>
        <button style={styles.modalCancel} onClick={() => { setDisputeModal(null); setDisputeReason(''); setDisputeCategory(''); setDisputeSecondary(''); setMissingTripId(''); setMissingTripInput(''); setMissingTripIds([]); setMissingTripError(''); setProofFile(null); setProofError('') }}>Cancel</button>
        <button style={{...styles.modalConfirm, background:'#d97706', opacity: disputeLoading ? 0.6 : 1}} onClick={submitDispute} disabled={disputeLoading}>
...
{bulkModal && (
  <div style={styles.modalOverlay}>
    <div style={{...styles.modal, maxWidth:'580px'}}>
      ...
      <p style={{fontSize:'0.82rem',color:'#6b7280',marginBottom:'1rem'}}>Upload an Excel file with multiple disputes and individual proof files for each row.</p>
      ...
      <div style={{background:'#f0f7ff',border:'1px solid #bfdbfe',borderRadius:'10px',padding:'0.75rem 1rem',marginBottom:'1rem',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        <div>
          <div style={{fontSize:'0.82rem',fontWeight:600,color:'#1e40af'}}>Step 1 — Download Template</div>
          <div style={{fontSize:'0.72rem',color:'#6b7280',marginTop:'0.2rem'}}>Fill in the Excel template with your dispute details</div>
        </div>
        <button style={{padding:'0.4rem 0.75rem',background:'#1e40af',border:'none',borderRadius:'8px',color:'#fff',fontFamily:'Inter,sans-serif',fontSize:'0.78rem',fontWeight:600,cursor:'pointer',whiteSpace:'nowrap'}} onClick={downloadTemplate}>
          ↓ Download Template
        </button>
      </div>

      {/* VALID CATEGORIES HINT */}
      <div style={{background:'#fffbeb',border:'1px solid #fcd34d',borderRadius:'10px',padding:'0.75rem 1rem',marginBottom:'1rem'}}>
        ...
        <div style={{fontSize:'0.72rem',color:'#6b7280',lineHeight:1.8}}>
          Wrong RFQ applied · Wrong Rate · Adhoc/Regular Mismatch · Vehicle size mismatch · Route Changed / Addition stops added · Vehicle number Mismatch · Trip Missing
        </div>
        <div style={{fontSize:'0.72rem',color:'#d97706',marginTop:'0.4rem',fontWeight:600}}>For Trip Missing: trip_id and secondary_answer must match TR-XXXXXXXX-XXXX format</div>
      </div>
      ...
      <div style={{background:'#f8fafc',border:'1px solid #e2e4ed',borderRadius:'10px',padding:'0.75rem 1rem',marginBottom:'0.75rem'}}>
        ...
        <input type="file" accept=".xlsx,.xls" style={{display:'none'}} id="bulkExcelInput"
          onChange={e => { const f = e.target.files[0]; if (f) setBulkExcel(f) }}/>
        <label htmlFor="bulkExcelInput" style={{display:'inline-flex',alignItems:'center',gap:'0.4rem',padding:'0.5rem 1rem',background:'#fff',border:'1px solid #e2e4ed',borderRadius:'8px',cursor:'pointer',fontSize:'0.82rem',color:'#4f46e5',fontWeight:600}}>
          📊 {bulkExcel ? 'Change Excel File' : 'Choose Excel File'}
        </label>
        {bulkExcel && (
          <div style={{display:'flex',alignItems:'center',gap:'0.5rem',marginTop:'0.5rem',padding:'0.4rem 0.75rem',background:'rgba(5,150,105,0.08)',border:'1px solid rgba(5,150,105,0.3)',borderRadius:'6px'}}>
            <span style={{fontSize:'0.78rem',color:'#059669',fontWeight:600}}>✓ {bulkExcel.name}</span>
            <span style={{fontSize:'0.75rem',color:'#6b7280'}}>({(bulkExcel.size/1024/1024).toFixed(2)}MB)</span>
            <span style={{cursor:'pointer',color:'#dc2626',marginLeft:'auto',fontWeight:700}} onClick={() => { setBulkExcel(null); document.getElementById('bulkExcelInput').value=''; }}>×</span>
          </div>
        )}
      </div>
      ...
      <div style={{background:'#f8fafc',border:'1px solid #e2e4ed',borderRadius:'10px',padding:'0.75rem 1rem',marginBottom:'0.75rem'}}>
        ...
        <div style={{fontSize:'0.72rem',color:'#6b7280',marginBottom:'0.5rem'}}>Upload all proof files (JPG/PNG/PDF). Filenames must match proof_filename column in Excel exactly.</div>
        <input type="file" accept=".jpg,.jpeg,.png,.pdf" multiple style={{display:'none'}} id="bulkProofInput"
          onChange={e => {
            const files = Array.from(e.target.files)
            const invalid = files.filter(f => f.size > 10 * 1024 * 1024)
            if (invalid.length > 0) { alert(`These files exceed 10MB: ${invalid.map(f=>f.name).join(', ')}`); return }
            setBulkProofs(files)
          }}/>
        <label htmlFor="bulkProofInput" style={{display:'inline-flex',alignItems:'center',gap:'0.4rem',padding:'0.5rem 1rem',background:'#fff',border:'1px solid #e2e4ed',borderRadius:'8px',cursor:'pointer',fontSize:'0.82rem',color:'#4f46e5',fontWeight:600}}>
          📎 {bulkProofs.length > 0 ? `Change Files (${bulkProofs.length} selected)` : 'Choose Proof Files'}
        </label>
        {bulkProofs.length > 0 && (
          <div style={{marginTop:'0.5rem',display:'flex',flexWrap:'wrap',gap:'0.4rem'}}>
            {bulkProofs.map((f,i) => (
              <div key={i} style={{background:'rgba(5,150,105,0.08)',border:'1px solid rgba(5,150,105,0.3)',borderRadius:'6px',padding:'0.2rem 0.5rem',fontSize:'0.72rem',color:'#059669',fontWeight:600}}>
                ✓ {f.name}
              </div>
            ))}
          </div>
      ...
      {bulkErrors.length > 0 && (
        <div style={{marginBottom:'0.75rem'}}>
          <div style={{fontSize:'0.82rem',fontWeight:700,color:'#dc2626',marginBottom:'0.5rem'}}>
            ✕ {bulkErrors.length} error{bulkErrors.length > 1 ? 's' : ''} found — please fix and re-upload
          </div>
      ...
      <div style={{display:'flex',gap:'0.75rem',marginTop:'0.5rem'}}>
        ...
        <button style={{...styles.modalConfirm, background:'#4f46e5', opacity: bulkLoading ? 0.6 : 1}} onClick={submitBulkDispute} disabled={bulkLoading}>
          {bulkLoading ? 'Uploading…' : 'Submit All Disputes'}
        </button>
      </div>
    </div>
  </div>
              ...
              <div style={{overflowX:'auto'}}>
                <table style={styles.table}>
                  <thead><tr style={{background:'#f0f1f5',borderBottom:'1px solid #e2e4ed'}}>
                    {['Transporter ID','Vendor Name','Oracle ID','Total Trips','Total Amount (₹)'].map(h => (
                  ...
                  <tbody>
                    {statsBar.map((s, i) => (
                      <tr key={i} style={{borderBottom:'1px solid #e2e4ed'}}>
                        <td style={{...styles.td,color:'#4f46e5',fontWeight:600}}>{s.transporter_id||'—'}</td>
                        <td style={styles.td}>{s.vendor_name||'—'}</td>
                        <td style={styles.td}>{s.oracle_id||'—'}</td>
                        <td style={{...styles.td,textAlign:'right',fontWeight:700,color:'#4f46e5'}}>{(s.total_trips||0).toLocaleString('en-IN')}</td>
                        <td style={{...styles.td,textAlign:'right',fontWeight:700,color:'#059669'}}>₹{(parseFloat(s.total_amount)||0).toLocaleString('en-IN',{maximumFractionDigits:2})}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
...
<div style={{...styles.tableWrap, marginTop:'1.5rem'}}>
  <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0.85rem 1rem',borderBottom:'1px solid #e2e4ed',background:'#f0f1f5'}}>
    <div style={{fontSize:'0.7rem',fontWeight:700,letterSpacing:'0.07em',textTransform:'uppercase',color:'#6b7280'}}>
      Pending Vendor Action — Ageing Report
    </div>
    <button style={{fontSize:'0.75rem',color:'#4f46e5',background:'none',border:'none',cursor:'pointer',fontWeight:600}} onClick={loadAgeing}>
      ↻ Refresh
    </button>
  </div>
  {ageingLoading ? (
    ...
    <>
      <div style={{padding:'0.75rem 1rem',borderBottom:'1px solid #e2e4ed',fontSize:'0.82rem',color:'#6b7280'}}>
        Total pending trips: <strong style={{color:'#1a1d2e'}}>{ageingData.totalPending?.toLocaleString('en-IN')}</strong>
      </div>
        ...
        {ageingData.buckets && Object.entries(ageingData.buckets).map(([key, bucket], i) => {
          ...
          return (
            <div key={key} style={{
              ...
              <div style={{fontSize:'2rem',fontWeight:800,color:c.text,lineHeight:1}}>
                {bucket.count.toLocaleString('en-IN')}
              </div>
              <div style={{fontSize:'0.72rem',fontWeight:600,color:c.text,marginTop:'0.4rem',lineHeight:1.3}}>
                {bucket.label}
              </div>
              {bucket.count > 0 && (
                <div style={{marginTop:'0.5rem',display:'inline-block',background:c.badge,border:`1px solid ${c.border}`,borderRadius:'6px',padding:'0.15rem 0.4rem',fontSize:'0.68rem',fontWeight:600,color:c.badgeText}}>
                  ₹{(bucket.amount/1000).toFixed(0)}K at risk
                </div>
              )}
            </div>
          )
      ...
      <div style={{padding:'0.6rem 1rem',background:'#f8fafc',borderTop:'1px solid #e2e4ed',fontSize:'0.72rem',color:'#6b7280'}}>
        ⚠ SLA breach = trips where vendor has not acted within 2 days of trip end date
      </div>
    </>
  ) : (
  ...
  <button style={styles.missingTripBtn} onClick={() => { setDisputeModal({trip_id:'MISSING',amount:0,vendor_name:vendorName}); setDisputeCategory('Trip Missing'); setDisputeSecondary('') }}>
    + Report Missing Trip
  </button>
  <button style={{...styles.missingTripBtn, borderColor:'#4f46e5', color:'#4f46e5'}} onClick={() => { setBulkModal(true); setBulkErrors([]); setBulkSuccess('') }}>
    ↑ Bulk Upload Disputes
                ...
                <select style={styles.select} value={transporterFilter} onChange={e => setTransporterFilter(e.target.value)}>
                  <option value="">All Transporter IDs</option>
                  {transporterIds.map(t => <option key={t} value={t}>{t}</option>)}
                </select>
                  ...
                  <table style={styles.table}>
                    ...
                    <tbody>
                      {filteredRows.length === 0 ? (
                        <tr><td colSpan={13} style={{textAlign:'center',padding:'3rem',color:'#6b7280'}}>No trips match your filters.</td></tr>
                        ...
                        return (
                          <tr key={i} style={{borderBottom:'1px solid #e2e4ed'}}>
                            ...
                            <td style={{...styles.td,minWidth:'140px'}}>
                              ...
                              {status === 'pending' && <>
                                <button style={styles.btnAccept} onClick={() => setPendingAccept({tripId:r.trip_id,transporterId:r.transporter_id,vendorName:r.vendor_name,amount:r.amount})}>✓ Accept Amount</button>
                                <button style={styles.btnDispute} onClick={() => { setDisputeModal(r); setDisputeReason('') }}>⚠ Raise Dispute</button>
                              </>}
                            </td>
                            <td style={styles.td}>
                              {res?.valmo_remarks ? <span style={styles['badge_'+res.valmo_remarks.toLowerCase().replace(/ /g,'_')] || styles.badgeNoDispute}>{res.valmo_remarks}</span> : <span style={styles.badgeNoDispute}>No dispute</span>}
                            </td>
                            <td style={{...styles.td,fontSize:'0.75rem',color:'#6b7280',maxWidth:'180px'}}>
  {res?.reason_for_rejection ? res.reason_for_rejection : '—'}
</td>
              ...
              {hasMore && (
                <div style={{textAlign:'center',marginTop:'1rem'}}>
                  <button style={styles.loadMoreBtn} onClick={() => loadData(page + 1)}>Load More →</button>
...
const styles = {
  ...
  btnDispute: { background:'rgba(217,119,6,0.08)',border:'1px solid rgba(217,119,6,0.3)',color:'#d97706',borderRadius:'8px',padding:'0.35rem 0.75rem',fontFamily:'Inter,sans-serif',fontSize:'0.75rem',fontWeight:600,cursor:'pointer',whiteSpace:'nowrap',display:'block',width:'100%',textAlign:'center',textDecoration:'none' },
  ...
  modal: { background:'#fff',border:'1px solid #e2e4ed',borderRadius:'16px',padding:'2rem',maxWidth:'380px',width:'100%' },
