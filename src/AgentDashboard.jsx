...
export default function AgentDashboard() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (!session) { navigate('/agent'); return }
  ...
  useEffect(() => {
    if (!userEmail) return
    loadDisputes(userEmail)
    loadAgeing(userEmail)
  }, [userEmail])

  async function loadDisputes(email) {
    try {
      ...
      if (err) { setError('Failed to load disputes.'); setLoading(false); return }
      setDisputes(data || [])
    } catch {
  ...
  async function loadAgeing(email) {
    try {
      ...
      const now = new Date()
      const buckets = {
        within8: { label: '0–8 Hours', count: 0, color: '#10b981' },
        within16: { label: '8–16 Hours', count: 0, color: '#f59e0b' },
        within24: { label: '16–24 Hours', count: 0, color: '#f97316' },
        beyond24: { label: '24+ Hours', count: 0, color: '#ef4444' }
      }
      ;(openDisputes || []).forEach(d => {
        if (!d.raised_at) return
        const hours = (now - new Date(d.raised_at)) / 3600000
        if (hours <= 8) buckets.within8.count++
        else if (hours <= 16) buckets.within16.count++
        else if (hours <= 24) buckets.within24.count++
        else buckets.beyond24.count++
      })
      setAgeingData({ success: true, totalOpen: (openDisputes || []).length, buckets })
    } catch {}
  }
  ...
  async function handleResolve() {
    ...
    if ((resolution === 'Approved' || resolution === 'Partially approved') && !finalAmount) {
      alert('Please enter the final approved amount.'); return
    }
    if (resolution === 'Approved' && parseFloat(finalAmount) > resolveModal.amount * 1.2) {
      alert(`Amount cannot exceed 120% of billed amount (₹${(resolveModal.amount * 1.2).toLocaleString('en-IN')}) for full approval.`); return
    }
    if (resolution === 'Partially approved' && parseFloat(finalAmount) > resolveModal.amount * 1.1) {
      alert(`Amount cannot exceed 110% of billed amount (₹${(resolveModal.amount * 1.1).toLocaleString('en-IN')}) for partial approval.`); return
    }
    ...
    const { error: err } = await supabase
      ...
      .update({
        status: 'resolved',
        valmo_remarks: resolution,
        reason_for_rejection: rejectionReason,
        final_amount_approved: parseFloat(finalAmount) || null,
        resolved_by: userEmail,
        resolved_at: new Date().toISOString()
      })
      .eq('trip_id', resolveModal.trip_id)
      .eq('assigned_to', userEmail)

    if (!err) {
      setDisputes(prev => prev.map(d =>
        d.trip_id === resolveModal.trip_id
          ? { ...d, status: 'resolved', valmo_remarks: resolution, reason_for_rejection: rejectionReason, final_amount_approved: finalAmount, resolved_by: userEmail }
          : d
      ))
  ...
  async function handleUploadMapping(e) {
    ...
    try {
      ...
      if (rows.length === 0) { setUploadMsg('No valid rows found.'); setUploading(false); return }
      ...
      setUploadMsg(insErr ? 'Upload failed: ' + insErr.message : `Uploaded ${rows.length} mappings successfully.`)
    } catch {
  ...
  async function downloadBulkTemplate() {
    try {
      const { data: openDisputes } = await supabase
        .from('disputes').select('*').eq('assigned_to', userEmail).eq('status', 'open')
      if (!openDisputes || openDisputes.length === 0) { alert('No open disputes to download.'); return }

      const rows = openDisputes.map(d => ({
        trip_id: d.trip_id,
      ...
      XLSX.writeFile(wb, 'bulk_resolution_template.xlsx')
    } catch {
    ...
    reader.onload = async (e) => {
      try {
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          if (!row.resolution) { errors.push({ row: rowNum, trip_id: row.trip_id, error: 'Missing resolution' }); continue }
          if (!['Approved','Partially approved','Rejected'].includes(row.resolution)) { errors.push({ row: rowNum, trip_id: row.trip_id, error: 'Invalid resolution value' }); continue }
          if (!row.reason) { errors.push({ row: rowNum, trip_id: row.trip_id, error: 'Missing reason' }); continue }

          const dispute = disputes.find(d => d.trip_id === row.trip_id)
          if (row.resolution === 'Approved' && row.final_amount && dispute && parseFloat(row.final_amount) > dispute.amount * 1.2) {
            errors.push({ row: rowNum, trip_id: row.trip_id, error: `Amount exceeds 120% cap (max ₹${(dispute.amount * 1.2).toLocaleString('en-IN')})` }); continue
          }
          if (row.resolution === 'Partially approved' && row.final_amount && dispute && parseFloat(row.final_amount) > dispute.amount * 1.1) {
            errors.push({ row: rowNum, trip_id: row.trip_id, error: `Amount exceeds 110% cap (max ₹${(dispute.amount * 1.1).toLocaleString('en-IN')})` }); continue
          }

          const { error: updErr } = await supabase.from('disputes').update({
            status: 'resolved',
            valmo_remarks: row.resolution,
            reason_for_rejection: row.reason || '',
            final_amount_approved: parseFloat(row.final_amount) || null,
            resolved_by: userEmail,
            resolved_at: new Date().toISOString()
          }).eq('trip_id', row.trip_id).eq('assigned_to', userEmail)

          if (updErr) errors.push({ row: rowNum, trip_id: row.trip_id, error: updErr.message })
          else resolved++
        }

        if (errors.length === 0) {
          setBulkResolveSuccess(`Successfully resolved ${resolved} disputes!`)
          setBulkResolveFile(null)
          loadDisputes(userEmail)
          loadAgeing(userEmail)
        } else {
          setBulkResolveErrors(errors)
        }
      } catch (err) {
        setBulkResolveErrors([{ row: '—', trip_id: '—', error: err.message }])
      }
      setBulkResolveLoading(false)
    }
    reader.readAsArrayBuffer(bulkResolveFile)
  }
  ...
  async function submitClaudePrompt() {
    ...
    if (!backendUrl) {
      setClaudeError('Claude service is not configured. Set VITE_BACKEND_URL in your environment.')
      return
    }
    ...
    try {
      const res = await fetch(`${backendUrl}/api/claude`, {
        ...
        body: JSON.stringify({ agentEmail: userEmail, prompt })
      })
      const data = await res.json()
      if (data.success) setClaudeResponse(data.answer || 'Claude did not return a response.')
      else setClaudeError(data.message || 'Claude request failed.')
    } catch {
    ...
    <div style={styles.layout}>
          ...
          <div style={styles.modal}>
            ...
            {resolution === 'Approved' && (
              <>
                ...
                <input style={{...styles.formInput, borderColor: finalAmount && parseFloat(finalAmount) > resolveModal.amount * 1.2 ? '#dc2626' : '#e2e4ed'}} type="number" placeholder="Enter amount…" value={finalAmount} onChange={e => setFinalAmount(e.target.value)}/>
                {finalAmount && !isNaN(parseFloat(finalAmount)) && parseFloat(finalAmount) > 0 && (
                  <div style={{fontSize:'0.75rem',color:'#4f46e5',marginTop:'0.3rem',fontWeight:600}}>₹{parseFloat(finalAmount).toLocaleString('en-IN')} = {amountToWords(parseFloat(finalAmount))}</div>
                )}
                {finalAmount && parseFloat(finalAmount) > resolveModal.amount * 1.2 && (
                  <div style={{marginTop:'0.4rem',padding:'0.5rem 0.75rem',background:'rgba(220,38,38,0.08)',border:'1px solid rgba(220,38,38,0.3)',borderRadius:'8px',fontSize:'0.78rem',color:'#dc2626',fontWeight:600}}>
                    ✕ Amount cannot exceed 120% of billed amount (₹{(resolveModal.amount * 1.2).toLocaleString('en-IN')}).
                  </div>
                )}
                {finalAmount && parseFloat(finalAmount) > 0 && parseFloat(finalAmount) < resolveModal.amount && parseFloat(finalAmount) <= resolveModal.amount * 1.2 && (
                  <div style={{marginTop:'0.4rem',padding:'0.5rem 0.75rem',background:'rgba(245,158,11,0.08)',border:'1px solid rgba(245,158,11,0.3)',borderRadius:'8px',fontSize:'0.78rem',color:'#d97706',fontWeight:600}}>
                    ⚠ Entered amount is less than original billed amount (₹{resolveModal.amount?.toLocaleString('en-IN')}). Please confirm.
                  </div>
                )}
            ...
            {resolution === 'Partially approved' && (
              <>
                <label style={styles.formLabel}>Final Approved Amount (₹) <span style={{color:'#dc2626'}}>*</span></label>
                <input style={{...styles.formInput, borderColor: finalAmount && parseFloat(finalAmount) > resolveModal.amount * 1.1 ? '#dc2626' : '#e2e4ed'}} type="number" placeholder="Enter amount…" value={finalAmount} onChange={e => setFinalAmount(e.target.value)}/>
                {finalAmount && !isNaN(parseFloat(finalAmount)) && parseFloat(finalAmount) > 0 && (
                  <div style={{fontSize:'0.75rem',color:'#4f46e5',marginTop:'0.3rem',fontWeight:600}}>₹{parseFloat(finalAmount).toLocaleString('en-IN')} = {amountToWords(parseFloat(finalAmount))}</div>
                )}
                {finalAmount && parseFloat(finalAmount) > resolveModal.amount * 1.1 && (
                  <div style={{marginTop:'0.4rem',padding:'0.5rem 0.75rem',background:'rgba(220,38,38,0.08)',border:'1px solid rgba(220,38,38,0.3)',borderRadius:'8px',fontSize:'0.78rem',color:'#dc2626',fontWeight:600}}>
                    ✕ Amount cannot exceed 110% of billed amount (₹{(resolveModal.amount * 1.1).toLocaleString('en-IN')}).
                  </div>
                )}
                {finalAmount && parseFloat(finalAmount) > 0 && parseFloat(finalAmount) < resolveModal.amount && (
                  <div style={{marginTop:'0.4rem',padding:'0.5rem 0.75rem',background:'rgba(245,158,11,0.08)',border:'1px solid rgba(245,158,11,0.3)',borderRadius:'8px',fontSize:'0.78rem',color:'#d97706',fontWeight:600}}>
                    ⚠ Entered amount is less than original billed amount. Please confirm.
                  </div>
                )}
          ...
          <div style={{...styles.modal,maxWidth:'560px'}}>
            ...
            <div style={{background:'#fffbeb',border:'1px solid #fcd34d',borderRadius:'10px',padding:'0.75rem 1rem',marginBottom:'1rem'}}>
              ...
              <div style={{fontSize:'0.72rem',color:'#6b7280',lineHeight:2}}>
                ...
                <div>• <strong>resolution</strong> must be exactly: <strong style={{color:'#059669'}}>Approved</strong> · <strong style={{color:'#d97706'}}>Partially approved</strong> · <strong style={{color:'#dc2626'}}>Rejected</strong></div>
                <div>• <strong>Approved</strong> → final_amount required, max 120% of billed</div>
                <div>• <strong>Partially approved</strong> → final_amount required, max 110% of billed</div>
            ...
            <div style={{background:'#f8fafc',border:'1px solid #e2e4ed',borderRadius:'10px',padding:'0.75rem 1rem',marginBottom:'0.75rem'}}>
              ...
              <input type="file" accept=".xlsx,.xls" style={{display:'none'}} id="bulkResolveInput" onChange={e => { setBulkResolveFile(e.target.files[0]); setBulkResolveErrors([]); setBulkResolveSuccess('') }}/>
              ...
              <div style={{marginBottom:'0.75rem'}}>
                ...
                <div style={{maxHeight:'180px',overflowY:'auto',border:'1px solid #fecaca',borderRadius:'8px'}}>
                  <table style={{width:'100%',borderCollapse:'collapse',fontSize:'0.75rem'}}>
                    <thead><tr style={{background:'#fef2f2'}}><th style={{padding:'0.4rem 0.75rem',textAlign:'left',color:'#dc2626'}}>Row</th><th style={{padding:'0.4rem 0.75rem',textAlign:'left',color:'#dc2626'}}>Trip ID</th><th style={{padding:'0.4rem 0.75rem',textAlign:'left',color:'#dc2626'}}>Error</th></tr></thead>
                    <tbody>{bulkResolveErrors.map((e,i) => (<tr key={i} style={{borderTop:'1px solid #fecaca'}}><td style={{padding:'0.4rem 0.75rem',color:'#6b7280'}}>{e.row}</td><td style={{padding:'0.4rem 0.75rem',color:'#4f46e5',fontWeight:600}}>{e.trip_id}</td><td style={{padding:'0.4rem 0.75rem',color:'#dc2626'}}>{e.error}</td></tr>))}</tbody>
                  </table>
                </div>
              </div>
            )}
      ...
      <aside style={styles.sidebar}>
          ...
          {ageingData ? (
            <>
              {Object.entries(ageingData.buckets).map(([key, bucket]) => (
                <div key={key} style={{background:'rgba(255,255,255,0.05)',borderRadius:'8px',padding:'0.5rem 0.75rem',marginBottom:'0.4rem',display:'flex',alignItems:'center',justifyContent:'space-between',border:`1px solid ${bucket.color}40`}}>
                  <div style={{fontSize:'0.72rem',color:'rgba(255,255,255,0.6)',lineHeight:1.3}}>{bucket.label}</div>
                  <div style={{fontSize:'1.2rem',fontWeight:800,color:bucket.color,minWidth:'28px',textAlign:'right'}}>{bucket.count}</div>
                </div>
              ))}
              <div style={{marginTop:'0.5rem',padding:'0.4rem 0.75rem',background:ageingData.buckets.beyond24.count>0?'rgba(220,38,38,0.15)':'rgba(255,255,255,0.05)',borderRadius:'8px',border:ageingData.buckets.beyond24.count>0?'1px solid rgba(220,38,38,0.4)':'1px solid rgba(255,255,255,0.1)',fontSize:'0.72rem',color:ageingData.buckets.beyond24.count>0?'#fca5a5':'rgba(255,255,255,0.4)',textAlign:'center',fontWeight:ageingData.buckets.beyond24.count>0?700:400}}>
                {ageingData.buckets.beyond24.count > 0 ? `🚨 ${ageingData.buckets.beyond24.count} overdue — action needed!` : '✓ All within SLA'}
              </div>
              <button style={{width:'100%',marginTop:'0.5rem',padding:'0.3rem',background:'transparent',border:'none',color:'rgba(255,255,255,0.3)',fontSize:'0.68rem',cursor:'pointer',fontFamily:'Inter,sans-serif'}} onClick={() => loadAgeing(email)}>↻ Refresh ageing</button>
            </>
          ) : (
        ...
        <div style={{padding:'0 1rem',marginTop:'0.5rem'}}>
          ...
          <div style={{fontSize:'0.68rem',color:'rgba(255,255,255,0.3)',marginTop:'0.4rem',lineHeight:1.4}}>CSV format:<br/>agent_email, transporter_id</div>
        </div>
      ...
      <div style={styles.mainArea}>
        <header style={styles.header}>
          ...
          <div style={styles.agentBadge}>{email}</div>
        </header>

        <div style={{padding:'1.5rem'}}>
          <div style={{display:'flex',gap:'0.5rem',marginBottom:'1rem'}}>
            {[['open','Open'],['resolved','Resolved'],['all','All']].map(([val,label]) => (
              <button key={val} style={{...styles.filterTab,...(filter===val?styles.filterTabActive:{})}} onClick={() => setFilter(val)}>{label}</button>
            ))}
            <button style={styles.refreshBtn} onClick={() => loadDisputes(email)}>↻ Refresh</button>
            <button style={{...styles.refreshBtn,marginLeft:'0',background:'#4f46e5',color:'#fff',border:'1px solid #4f46e5',fontWeight:600}} onClick={() => { setBulkResolveModal(true); setBulkResolveErrors([]); setBulkResolveSuccess('') }}>⚡ Bulk Resolve</button>
          </div>
          ...
          <div style={{marginBottom:'1rem',padding:'1rem',border:'1px solid #e2e4ed',borderRadius:'16px',background:'#fff',boxShadow:'0 1px 4px rgba(0,0,0,0.05)'}}>
            ...
            <textarea style={{...styles.formTextarea,minHeight:'100px'}} placeholder="Enter your question for Claude..." value={claudePrompt} onChange={e => setClaudePrompt(e.target.value)}/>
            <div style={{display:'flex',gap:'0.75rem',marginTop:'0.75rem',alignItems:'center'}}>
              ...
              <button style={styles.modalCancel} onClick={() => { setClaudePrompt(''); setClaudeResponse(''); setClaudeError('') }}>Clear</button>
            </div>
          ...
          <div style={styles.tableWrap}>
            <div style={{overflowX:'auto'}}>
              <table style={styles.table}>
                ...
                <tbody>
                  {filtered.length === 0 ? (
                    <tr><td colSpan={11} style={{textAlign:'center',padding:'3rem',color:'#6b7280'}}>No {filter === 'all' ? '' : filter} disputes found.</td></tr>
                  ) : filtered.map((d, i) => (
                    <tr key={i} style={{borderBottom:'1px solid #e2e4ed'}}>
                      ...
                      <td style={styles.td}>
                        {d.status === 'open' ? <span style={styles.badgeOpen}>Open</span> : <span style={styles.badgeResolved}>Resolved</span>}
                      </td>
                      <td style={styles.td}>
                        {d.proof_file
                          ? <a href={d.proof_file} target="_blank" rel="noreferrer" style={{display:'inline-flex',alignItems:'center',gap:'0.3rem',padding:'0.3rem 0.6rem',background:'rgba(79,70,229,0.08)',border:'1px solid rgba(79,70,229,0.2)',borderRadius:'6px',color:'#4f46e5',fontSize:'0.75rem',fontWeight:600,textDecoration:'none',whiteSpace:'nowrap'}}>📎 View Proof</a>
                          : <span style={{fontSize:'0.75rem',color:'#6b7280'}}>No proof</span>}
                      </td>
                      <td style={styles.td}>
                        {d.valmo_remarks
