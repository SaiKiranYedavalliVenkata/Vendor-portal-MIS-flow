...
const [bulkResolveModal, setBulkResolveModal] = useState(false)
  ...
  useEffect(() => {
    if (!email) { navigate('/admin'); return }
    loadAgents()
  }, [])
  ...
  async function loadPerformance() {
    setLoading(true)
    const res = await axios.post(`${API}/api/admin/agent-performance`, { dateFrom, dateTo })
  ...
  async function loadDisputes(pg) {
    setLoading(true)
    const res = await axios.post(`${API}/api/admin/all-disputes`, {
      agentFilter, statusFilter, page: pg
    })
    if (res.data.success) {
      if (pg === 0) setAllDisputes(res.data.disputes)
      else setAllDisputes(prev => [...prev, ...res.data.disputes])
      setDisputeTotal(res.data.total)
      setDisputeHasMore(res.data.hasMore)
      setDisputePage(pg)
    }
    setLoading(false)
  }
  ...
  async function loadVendors() {
    setLoading(true)
    const res = await axios.get(`${API}/api/admin/vendors`)
  ...
  async function uploadTrips() {
    ...
    const res = await axios.post(`${API}/api/admin/upload-trips`, formData, {
      headers: { 'Content-Type': 'multipart/form-data' }
    })
    setTripUploadResult(res.data)
  ...
  async function uploadMapping() {
    ...
    const res = await axios.post(`${API}/api/admin/upload-mapping`, formData, {
      headers: { 'Content-Type': 'multipart/form-data' }
    })
    setMappingMsg(res.data.message || 'Done')
    setMappingUploading(false)
  }

  async function addVendor() {
    if (!newVendorEmail || !newVendorTid) { setVendorMsg('Email and Transporter ID are required.'); return }
    const res = await axios.post(`${API}/api/admin/add-vendor`, {
      email: newVendorEmail, vendorName: newVendorName, transporterId: newVendorTid
    })
    if (res.data.success) {
      ...
      setVendorMsg(res.data.message)
    }
  }

  async function removeVendor(email, transporterId) {
    if (!window.confirm(`Remove ${email} (${transporterId})?`)) return
    await axios.post(`${API}/api/admin/remove-vendor`, { email, transporterId })
    loadVendors()
  }
    ...
    <div style={styles.layout}>
      ...
      <aside style={styles.sidebar}>
        ...
        <div style={styles.sidebarFooter}>
          ...
          <button style={styles.logoutBtn} onClick={() => navigate('/admin')}>Log Out</button>
        </div>
      </aside>
        ...
        <div style={{padding:'1.5rem'}}>
            ...
            <>
              ...
              <div style={{display:'flex',gap:'0.5rem',marginBottom:'1rem',flexWrap:'wrap',alignItems:'center'}}>
                {[['all','All Time'],['today','Today'],['7days','Last 7 Days'],['30days','Last 30 Days'],['custom','Custom']].map(([key,label]) => (
                  <button key={key} style={{...styles.filterTab, ...(quickFilter===key ? styles.filterTabActive : {})}} onClick={() => applyQuickFilter(key)}>
                    {label}
                  </button>
                ))}
                {quickFilter === 'custom' && (
          ...
          {activePage === 'disputes' && (
            <>
              <div style={{display:'flex',gap:'0.75rem',marginBottom:'1rem',flexWrap:'wrap'}}>
                <select style={styles.select} value={agentFilter} onChange={e => { setAgentFilter(e.target.value); loadDisputes(0) }}>
                  <option value="">All Agents</option>
                  {agents.map(a => <option key={a.email} value={a.email}>{a.name}</option>)}
                </select>
                <select style={styles.select} value={statusFilter} onChange={e => { setStatusFilter(e.target.value); loadDisputes(0) }}>
                    ...
                    <tbody>
                      {loading ? (
                        ...
                        <tr key={i} style={{borderBottom:'1px solid #e2e4ed'}}>
                          ...
                          <td style={{...styles.td,whiteSpace:'nowrap',fontSize:'0.78rem'}}>{d.raised_at?.slice(0,16)}</td>
          ...
          {activePage === 'trips' && (
            <div style={styles.uploadCard}>
              ...
              <p style={styles.uploadDesc}>Upload an Excel or CSV file to add new trips to the system. Existing trip IDs will be skipped.</p>
              <div style={styles.uploadBox}>
                <input type="file" accept=".xlsx,.xls,.csv" style={{display:'none'}} id="tripFileInput"
                  onChange={e => { setTripFile(e.target.files[0]); setTripUploadResult(null) }}/>
                <label htmlFor="tripFileInput" style={styles.chooseBtn}>
                  📊 {tripFile ? tripFile.name : 'Choose Excel or CSV File'}
                </label>
                {tripFile && (
                  <div style={{fontSize:'0.78rem',color:'#6b7280',marginTop:'0.5rem'}}>
                    {(tripFile.size/1024/1024).toFixed(2)}MB
                  </div>
                )}
              </div>
                ...
                <div style={{marginTop:'1rem',padding:'1rem',background: tripUploadResult.success ? '#f0fdf4' : '#fef2f2', border: `1px solid ${tripUploadResult.success ? '#bbf7d0' : '#fecaca'}`, borderRadius:'10px'}}>
                  {tripUploadResult.success ? (
                    <>
                      ...
                      <div style={{fontSize:'0.85rem',color:'#6b7280'}}>Skipped: {tripUploadResult.skipped} rows</div>
          ...
          {activePage === 'mapping' && (
            <div style={styles.uploadCard}>
              ...
              <p style={styles.uploadDesc}>Upload a CSV file with agent_email and transporter_id columns. This will replace the existing mapping.</p>
              ...
              <div style={styles.uploadBox}>
                <input type="file" accept=".csv" style={{display:'none'}} id="mappingFileInput"
                  onChange={e => { setMappingFile(e.target.files[0]); setMappingMsg('') }}/>
                <label htmlFor="mappingFileInput" style={styles.chooseBtn}>
            ...
            <>
              ...
              <div style={styles.tableWrap}>
                ...
                <table style={styles.table}>
                  <thead>
                    <tr style={{background:'#f0f1f5',borderBottom:'1px solid #e2e4ed'}}>
                      {['Email','Vendor Name','Transporter ID','Action'].map(h => (
                        <th key={h} style={styles.th}>{h}</th>
                      ))}
                    </tr>
                  </thead>
...
const styles = {
  ...
  adminBadge: { background:'#fef2f2',border:'1px solid #fecaca',borderRadius:'8px',padding:'0.3rem 0.75rem',fontSize:'0.8rem',fontWeight:600,color:'#dc2626' },
