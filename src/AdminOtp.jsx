...
export default function AdminOtp() {
  ...
  async function verifyOTP(otpVal) {
    ...
    try {
      const res = await axios.post(`${API}/api/admin/verify-otp`, { email, otp: code })
      if (res.data.success) {
        setMsg({ type:'ok', text:'Verified! Loading admin dashboard…' })
        navigate('/admin/dashboard', { state: { email, adminName: res.data.adminName } })
      } else {
    ...
    <div style={styles.body}>
      <div style={styles.wrap}>
        <div style={styles.brand}>
          ...
          <h1 style={styles.h1}>Admin Verification</h1>
        ...
        <p style={styles.foot}>
          <span style={{color:'#dc2626',cursor:'pointer'}} onClick={() => navigate('/admin')}>← Back</span>
        </p>
      </div>
    </div>
  )
